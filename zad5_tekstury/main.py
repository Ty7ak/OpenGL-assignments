...
#  Tekstury
textures = []
current_texture = "Textures/D1_t.tga"
texture_id = 0
...
def render_scene():
    ...
    import_texture(current_texture)
    egg()
    glFlush()
    glutSwapBuffers()
...
#  Funkcja pobiera teksture o podanej sciezce
def import_texture(path):
    img = Image.open(path)
    img_data = numpy.array(list(img.getdata()), numpy.int8)
    width, height = img.size

    imported_texture = glGenTextures(1)
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1)
    glBindTexture(GL_TEXTURE_2D, imported_texture)
...
#  Funkcja pobiera wszystkie sciezki tekstur do zmiennej textures[]
def get_texture_paths(folder):
    for filename in os.listdir(folder):
        textures.append("Textures/" + filename)


#  Funkcja zmieniajaca teksture
def change_texture(t=3):
...
#  Wątek obsługujący zmianę tekstur
...
def main():
    ...
    glutCreateWindow("Zadanie 5. - Tekstury")
